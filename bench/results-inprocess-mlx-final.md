...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 93% | 84% | 0.21s | 1.72s | 2.34s | 1/27 | 5/27 | 4/27 |

## Per case

**Qwen3-0.6B-MLX-4bit / short_question** 0.06s 56% recall 60% missing 'link'; retained 50% < 60%

> Yeah, wait what's in this PR?

**Qwen3-0.6B-MLX-4bit / long_ramble** 0.71s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / jargon** 0.16s 67% recall 100% missing '.env'

> the work tree is dirty, so i had to stash before rebasing onto main, and the dot env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.09s 67% recall 100% kept 'at 2'; kept 'actually'

> Let's meet at 2 actually 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.30s 100% recall 94% ok

> Okay so I tested the new build and the login flow is mostly working, but I found two issues. One is the spinner never goes away on slow connections. The other is when you log out it doesn't actually clear the session. We should fix those before before Friday.
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.25s 71% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'like closer'; LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.09s 95% recall 100% LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.21s 75% recall 91% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restated_clause** 0.08s 87% recall 80% kept 'actually'
...
**Qwen3-0.6B-MLX-4bit / snap_regions_joined** 0.55s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-0.6B-MLX-4bit / long_doc** 2.34s 81% recall 89% **over 1992ms budget** missing 'hundred and forty'; missing 'three hundred'; missing 'remembering it'

> Okay so for the design doc, we want to start with the problem: our dictation is accurate when you speak clearly, but falls apart when you mumble or trail off. The cleanup stage can't fix that because by then the words are already wrong. Next section: what we actually measured. The transcription tail is about 140 milliseconds, and the cleanup call is around 300. Neither of those is the bottleneck, the bottleneck is recognition quality. Next: the three options we discussed. They are biasing the recognizer with a vocabulary list, feeding the focused field's text in as context, and letting the user correct a word once and remember it. Then the recommendation: we do the third one because it's the only one that gets better as we use it and we should ship it behind a flag so we can measure it on real dictations before turning it on for everyone.

**Qwen3-0.6B-MLX-4bit / long_meeting_notes** 0.94s 100% recall 100% ok
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.43s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.53s 75% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.75s 85% recall 96% missing 'intent'

> So we have two other agents working on Blue Jay Whisper. I want you to mainly be concerned about the quality of how our transcription works because there's a lot of times where I say a lot of text and it doesn't take into account that I'm trying to prompt. So council our quality issues. You get a council of agents by using the debate skill and our prompt. Our system prompt and how our design works to give the best transcription quality. Imagine you're whisper flow and we're trying to replicate whisper flow one to one.

**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.31s 95% recall 100% missing 'actually'

> for the storage layer, we could use postgres with a jsonb column, or actually maybe duckdb since it's all analytical. or we could keep it in sqlite and deal with it later. I'm leaning postgres, but tell me if I'm wrong.

**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.25s 50% recall 100% missing '--force'; missing '.env'; missing 'app.log'
