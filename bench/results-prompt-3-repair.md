...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 81% | 0.33s | 1.97s | 4.75s | 1/30 | 7/30 | 4/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 1.05s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / jargon** 0.21s 67% recall 100% missing '.env'

> The work tree is dirty. I had to stash before rebasing onto main and the dot env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.33s 100% recall 100% ok

> Let's meet at 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.41s 100% recall 92% ok
...
**Qwen3-0.6B-MLX-4bit / question_not_instruction** 0.19s 100% recall 100% ok

> Hey, can you review my PR when you get a chance, it's the one that fixes the webhook retry logic.

**Qwen3-0.6B-MLX-4bit / discourse_like** 0.54s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.27s 100% recall 83% ok

> We should just ship it today and see what breaks.

**Qwen3-0.6B-MLX-4bit / meaningful_like** 0.18s 100% recall 100% ok

> I like the new design and it looks like a bug in the retry logic. Can you do something like that for the webhook one.

**Qwen3-0.6B-MLX-4bit / false_start** 0.50s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.32s 50% recall 100% kept 'to the left'; kept 'sorry'

> Can you move the button to the right side of the toolbar, sorry, to the left side?

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.13s 60% recall 90% missing 'thursday'; kept 'friday'

> We should ship it on Friday, and before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.36s 100% recall 83% ok

> The retry logic needs to check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.62s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"
...
**Qwen3-0.6B-MLX-4bit / snap_regions_joined** 0.62s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-0.6B-MLX-4bit / long_doc** 1.35s 83% recall 92% missing 'remembering it'
...
**Qwen3-0.6B-MLX-4bit / long_meeting_notes** 0.98s 100% recall 99% ok
...
**Qwen3-0.6B-MLX-4bit / draft_context** 0.18s 67% recall 100% missing 'FLAG_RETRY_V2'

> So far no errors in Sentry and the flag retry v2 flag is on for about 10 percent of orgs.

**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.72s 91% recall 100% LOSSY (dropped the last thing said): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.56s 80% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.84s 83% recall 98% missing 'system prompt'

> So we have two other agents working on Blue Jay Whisper, but I want you to mainly be concerned about the quality of how our transcription works because there's a lot of times where I say a lot of text and it doesn't take into account that I'm trying to prompt maybe I'm trying to prompt code and it just changes my text too much and it doesn't fully understand my intent. So council our quality issues can you get a council of agents by using the debate skill and our prompt, and how our design works to give the best transcription quality. Imagine you're whisper flow and we're trying to replicate whisper flow one to one.

**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.33s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.27s 67% recall 100% missing '--force'; missing 'app.log'

> Check whether flag retry v2 is set in the .env file and if it is, run npm run build dash dash force and tail the logs in var log slash app dot log.

**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 4.75s 90% recall 100% **over 2034ms budget** LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / deliberate_repetition** 0.07s 100% recall 100% ok

> Blue Jays, Blue Jays, Blue Jays

**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.33s 67% recall 75% missing 'X_audio'; kept 'X underscore'

> Should our paper mention X underscore audio if we never use it?

**Qwen3-0.6B-MLX-4bit / mishear_frontend_link** 0.09s 20% recall 100% missing 'frontend'; missing 'locally'; kept 'front number'; kept 'cycle'
...
**Qwen3-0.6B-MLX-4bit / mishear_dev_prod** 0.12s 38% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; missing 'prod'; missing 'middleware'; kept 'proud'; kept 'devin'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
