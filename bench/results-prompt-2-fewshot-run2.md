...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 98% | 84% | 0.27s | 1.62s | 1.72s | 0/30 | 6/30 | 3/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 0.99s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / jargon** 0.17s 67% recall 100% missing '.env'
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.59s 77% recall 98% kept 'like closer'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.28s 62% recall 93% kept 'sorry'
...
**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.42s 75% recall 100% kept 'needs to, we'

> The retry logic needs to, we need to first check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.59s 62% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'dragon'; kept "shouldn't even be showing"; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.72s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.53s 86% recall 100% kept 'you know'
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.83s 100% recall 100% ok
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.26s 67% recall 100% missing '.env'; missing 'app.log'

> Check whether flag retry v2 is set in the dot env file and if it is, run npm run build --force and tail the logs in var log / app / dot log.

**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 1.67s 90% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.35s 100% recall 83% ok
