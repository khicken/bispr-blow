...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 83% | 0.26s | 1.94s | 2.41s | 1/30 | 11/30 | 4/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 0.83s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / bug_report** 0.48s 94% recall 98% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.51s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.48s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.29s 62% recall 93% kept 'sorry'
...
**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.34s 100% recall 83% ok

> The retry logic needs to check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.52s 62% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'dragon'; kept "shouldn't even be showing"; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / long_doc** 1.85s 92% recall 97% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.08s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.60s 75% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 2.41s 90% recall 100% **over 2034ms budget** LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.32s 100% recall 67% ok
