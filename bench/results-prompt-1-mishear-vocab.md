...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 83% | 0.30s | 2.15s | 3.59s | 1/30 | 8/30 | 3/30 |
...
**Qwen3-0.6B-MLX-4bit / self_correction** 0.26s 100% recall 100% ok

> Let's meet at 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.38s 100% recall 96% ok
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.50s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.27s 100% recall 92% ok
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.53s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.30s 62% recall 93% kept 'sorry'

> Can you move the button to the right side of the toolbar, sorry?

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.11s 100% recall 80% ok

> We should ship it on Thursday, and before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.41s 88% recall 92% kept 'needs to, we'

> The retry logic needs to check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.70s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"
...
**Qwen3-0.6B-MLX-4bit / snap_regions_joined** 0.68s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-0.6B-MLX-4bit / long_doc** 1.62s 92% recall 97% missing 'remembering it'
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.77s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.57s 75% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.39s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.40s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 3.59s 90% recall 100% **over 2034ms budget** LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.34s 100% recall 67% ok
...
**Qwen3-0.6B-MLX-4bit / mishear_dev_prod** 0.15s 50% recall 93% missing 'prod'; missing 'middleware'; kept 'proud'; kept 'devin'
