...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 82% | 0.33s | 1.78s | 1.92s | 0/30 | 9/30 | 3/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 1.13s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / jargon** 0.19s 67% recall 80% missing '.env'
...
**Qwen3-0.6B-MLX-4bit / question_not_instruction** 0.18s 100% recall 100% ok

> Hey, can you review my PR when you get a chance? It's the one that fixes the webhook retry logic.

**Qwen3-0.6B-MLX-4bit / discourse_like** 0.57s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.30s 93% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead

> I think we should ship it today and see what breaks.

**Qwen3-0.6B-MLX-4bit / meaningful_like** 0.23s 100% recall 100% ok

> I like the new design and it looks like a bug in the retry logic. Can you do something like that for the webhook one?

**Qwen3-0.6B-MLX-4bit / false_start** 0.56s 68% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'like center'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / restated_clause** 0.12s 80% recall 90% kept 'friday'; kept 'actually'

> We should ship it on Friday, and on Thursday actually, before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.39s 88% recall 92% kept 'needs to, we'
...
**Qwen3-0.6B-MLX-4bit / snap_regions_joined** 0.36s 67% recall 100% kept 'dragon'; kept "shouldn't even be showing"

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-0.6B-MLX-4bit / long_doc** 1.43s 96% recall 97% missing 'remembering it'
...
**Qwen3-0.6B-MLX-4bit / long_meeting_notes** 0.57s 96% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.92s 91% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.10s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.35s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / mishear_dev_prod** 0.14s 43% recall 100% missing 'prod'; missing 'middleware'; kept 'proud'; kept 'devin'
