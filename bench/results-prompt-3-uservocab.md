...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 82% | 0.35s | 1.77s | 1.91s | 1/30 | 6/30 | 2/30 |
...
**Qwen3-0.6B-MLX-4bit / self_correction** 0.35s 50% recall 100% kept 'at 2'; kept 'actually'

> Let's meet at 2 pm, actually 3 pm, tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 1.26s 88% recall 100% **over 1248ms budget** LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.63s 77% recall 98% kept 'like closer'

> Our pill also sits a bit too high when it could be like closer to, let's say, the system tray on a desktop or like the task bar on a desktop or lower down in the ID or close to the bottom of the screen. It's just too far up and in the middle of everything.

**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.30s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.54s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.34s 75% recall 86% kept 'sorry'

> Can you move the button to the right side of the toolbar, sorry?

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.11s 60% recall 80% missing 'thursday'; kept 'friday'

> We should ship it on Friday, and before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.40s 100% recall 83% ok

> the retry logic needs to check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.69s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.38s 80% recall 100% kept 'you know'; LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 1.09s 92% recall 95% missing 'system prompt'
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.31s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 1.51s 100% recall 100% ok
