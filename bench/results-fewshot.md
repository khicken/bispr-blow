...
|---|---|---|---|---|---|---|---|---|
| Qwen3-1.7B-MLX-8bit | 97% | 88% | 0.65s | 4.34s | 4.59s | 9/27 | 1/27 | 1/27 |
| Qwen3-0.6B-MLX-4bit | 93% | 86% | 0.17s | 1.09s | 1.21s | 0/27 | 7/27 | 2/27 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 0.88s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / terminal_flags** 0.14s 100% recall 100% ok

> Run the tests with --verbose, and then commit and push to dev, not prod.

**Qwen3-0.6B-MLX-4bit / jargon** 0.17s 67% recall 93% missing '.env'

> The worktree is dirty. I had to stash before rebasing onto main and the dot env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.09s 100% recall 100% ok

> Let's meet at 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.37s 96% recall 97% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / question_not_instruction** 0.17s 100% recall 100% ok

> Hey, can you review my PR when you get a chance? It's the one that fixes the webhook retry logic.

**Qwen3-0.6B-MLX-4bit / discourse_like** 0.28s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.09s 95% recall 89% LOSSY (dropped content): guard rejected it, rules ran instead

> So i think we should just ship it today and see what breaks

**Qwen3-0.6B-MLX-4bit / meaningful_like** 0.19s 100% recall 100% ok

> I like the new design and it looks like a bug in the retry logic. So can you do something like that for the webhook one?

**Qwen3-0.6B-MLX-4bit / false_start** 0.27s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.10s 100% recall 71% ok

> Can you move the button to the right side of the toolbar?

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.09s 100% recall 80% ok
...
**Qwen3-0.6B-MLX-4bit / snap_regions** 0.66s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.55s 79% recall 100% kept 'you know'
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.31s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.26s 48% recall 100% missing '--force'; missing '.env'; missing 'app.log'
...
**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 0.97s 89% recall 87% missing 'twenty four hours'

> Okay, so for the reliability section, we start with the webhook sender failing, we just log it and move on. The customer never finds out and neither do we until they email us. Then the second part is what we're proposing: a dead letter table plus a daily digest that goes to the the org owner. Then the third part is about the tradeoff: a digest is not real time. If something breaks at nine in the morning, they hear about it the next day. I think that's acceptable for now because the alternative is a whole alerting system and we don't have anyone to build that this quarter. Then last the rollout plan: we ship the table first behind a flag measure how often it actually fills up on real traffic and only then decide whether the digest is even worth building.

**Qwen3-0.6B-MLX-4bit / deliberate_repetition** 0.08s 100% recall 100% ok

> Blue Jays, Blue Jays, Blue Jays

**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.10s 100% recall 67% ok

> Should our paper mention X_audio if we never use it.

**Qwen3-1.7B-MLX-8bit / short_question** 0.28s 100% recall 100% ok

> Yeah wait what's in this PR can you see the link again

**Qwen3-1.7B-MLX-8bit / long_ramble** 2.08s 88% recall 100% **over 1560ms budget** REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead

> We should also update the system prompt to not just strictly remove filler words but also tailor that is towards like developers so when we set up like accounts and stuff we can tailor it's more of a developer or a designer whatever the other categories you can think of put like 5 default options during onboarding but we also have to inject more context of the app and at the beginning of the system prompt or the end of it because i'm noticing that when i say for example dev or prod keeps saying like devin or prod or proud instead of dev or prod

**Qwen3-1.7B-MLX-8bit / terminal_flags** 0.39s 100% recall 100% ok

> Run the tests with --verbose, and then commit and push to dev, not prod.

**Qwen3-1.7B-MLX-8bit / jargon** 0.49s 100% recall 100% ok

> The work tree is dirty, so I had to stash before rebasing onto main and the .env file was missing.

**Qwen3-1.7B-MLX-8bit / self_correction** 0.32s 50% recall 100% kept 'at 2'; kept 'actually'

> Let's meet at 2 actually 3 pm tomorrow at the office.

**Qwen3-1.7B-MLX-8bit / bug_report** 1.13s 86% recall 100% kept 'the the'
...
**Qwen3-1.7B-MLX-8bit / question_not_instruction** 0.43s 100% recall 100% ok

> Hey can you review my PR when you get a chance it's the one that fixes the webhook retry logic

**Qwen3-1.7B-MLX-8bit / discourse_like** 1.12s 89% recall 86% kept 'like closer'

> Our pill still sits a bit too high, when it could be like closer to the system tray on a desktop or the task bar on a desktop or lower down in the ID or close to the bottom of the screen. It's just too far up and in the middle of everything.

**Qwen3-1.7B-MLX-8bit / stacked_fillers** 0.26s 100% recall 83% ok

> So we should just ship it today and see what breaks.

**Qwen3-1.7B-MLX-8bit / meaningful_like** 0.56s 100% recall 100% ok

> I like the new design and it looks like a bug in the retry logic. Can you do something like that for the webhook one

**Qwen3-1.7B-MLX-8bit / false_start** 0.79s 75% recall 100% kept 'like center'

> And by it's dragging to left or right, I mean, it's going to anchor to the like center, vertically centered, left side, or vertically centered right side of the screen.

**Qwen3-1.7B-MLX-8bit / restart_wrong_word** 0.40s 50% recall 100% kept 'to the left'; kept 'sorry'

> Can you move the button to the left, sorry, to the right side of the toolbar

**Qwen3-1.7B-MLX-8bit / restated_clause** 0.42s 60% recall 100% kept 'friday'; kept 'actually'

> We should ship it on Friday, we should ship it on Thursday actually, before the demo.

**Qwen3-1.7B-MLX-8bit / abandoned_phrase** 0.38s 100% recall 92% ok

> The retry logic needs to first check whether the webhook even fired before we retry anything.

**Qwen3-1.7B-MLX-8bit / snap_regions** 1.87s 71% recall 100% **over 1362ms budget** kept 'dragon'; kept "shouldn't even be showing"

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing. It shouldn't even be shown the pill is being picked up.

**Qwen3-1.7B-MLX-8bit / snap_regions_joined** 1.92s 71% recall 100% **over 1362ms budget** kept 'dragon'; kept "shouldn't even be showing"

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-1.7B-MLX-8bit / long_doc** 3.47s 100% recall 100% **over 1992ms budget** ok
...
**Qwen3-1.7B-MLX-8bit / agent_prompt_ui_options** 1.65s 86% recall 92% **over 1380ms budget** kept 'you know'

> Note latency is a huge factor. We want sub 500 to 600 milliseconds in the UI. I'm thinking there's model options like fastest, medium, and slow or something like that to show if someone wants to make the choice between three different choices. Or it could just be two choices between like you know fast but less accurate but this is or it's like slower and more accurate whatever you think is the best UI.

**Qwen3-1.7B-MLX-8bit / agent_prompt_council** 2.32s 100% recall 96% **over 1626ms budget** ok
...
**Qwen3-1.7B-MLX-8bit / enumerated_options** 0.93s 100% recall 100% ok

> For the storage layer we could use postgres with a jsonb column or actually maybe DuckDB since it's all analytical, or we could just keep it in sqlite and deal with it later. I'm leaning postgres but tell me if I'm wrong.

**Qwen3-1.7B-MLX-8bit / identifier_symbols** 0.65s 100% recall 81% ok
...
**Qwen3-1.7B-MLX-8bit / design_doc_polish_long** 3.44s 89% recall 100% **over 2034ms budget** kept 'we we'
...
**Qwen3-1.7B-MLX-8bit / spoken_underscore** 0.28s 100% recall 83% ok
