...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 99% | 81% | 0.33s | 1.61s | 1.82s | 0/30 | 6/30 | 3/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 1.01s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / self_correction** 0.32s 75% recall 100% kept 'at 2'

> Let's meet at 2 and 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.31s 94% recall 98% LOSSY (dropped the last thing said): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.53s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.28s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.34s 50% recall 100% kept 'to the left'; kept 'sorry'

> Can you move the button to the left, sorry, to the right side of the toolbar.

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.14s 60% recall 100% kept 'friday'; kept 'actually'

> We should ship it on Friday, and ship it on Thursday actually, before the demo.
...
**Qwen3-0.6B-MLX-4bit / snap_regions** 0.67s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.82s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.57s 86% recall 100% kept 'you know'
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.87s 100% recall 100% ok
