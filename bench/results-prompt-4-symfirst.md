...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 98% | 78% | 0.15s | 0.68s | 1.10s | 0/30 | 12/30 | 2/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 0.62s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / jargon** 0.15s 100% recall 100% ok
...
**Qwen3-0.6B-MLX-4bit / bug_report** 0.28s 94% recall 94% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.35s 71% recall 100% kept 'like closer'; LOSSY (dropped the last thing said): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.18s 50% recall 100% kept 'to the left'; kept 'sorry'
...
**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.17s 100% recall 83% ok
...
**Qwen3-0.6B-MLX-4bit / draft_context** 0.16s 67% recall 94% missing 'FLAG_RETRY_V2'

> So far no errors in sentry and the flag retry v2 flag is on for about 10 percent of orgs.

**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 0.21s 91% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.47s 75% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.19s 50% recall 100% missing '--force'; missing '.env'; missing 'app.log'
...
**Qwen3-0.6B-MLX-4bit / deliberate_repetition** 0.05s 100% recall 100% ok

> Blue Jays, Blue Jays, Blue Jays

**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.14s 33% recall 83% missing 'X_audio'; kept 'X underscore'

> Should our paper mention X underscore audio if we never use it?

**Qwen3-0.6B-MLX-4bit / mishear_frontend_link** 0.06s 20% recall 100% missing 'frontend'; missing 'locally'; kept 'front number'; kept 'cycle'
...
**Qwen3-0.6B-MLX-4bit / mishear_dev_prod** 0.10s 43% recall 100% missing 'prod'; missing 'middleware'; kept 'proud'; kept 'devin'
