...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 96% | 83% | 0.28s | 1.47s | 2.20s | 0/30 | 7/30 | 3/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 0.91s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / jargon** 0.18s 67% recall 90% missing '.env'

> The work tree is dirty. So I had to stash before rebasing onto main, and the dot env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.24s 100% recall 100% ok

> Let's meet at 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.32s 100% recall 96% ok
...
**Qwen3-0.6B-MLX-4bit / question_not_instruction** 0.16s 100% recall 100% ok

> Hey, can you review my PR when you get a chance? It's the one that fixes the webhook retry logic.

**Qwen3-0.6B-MLX-4bit / discourse_like** 0.48s 71% recall 100% kept 'like closer'; LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.24s 100% recall 92% ok
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.49s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.29s 50% recall 100% kept 'to the left'; kept 'sorry'
...
**Qwen3-0.6B-MLX-4bit / snap_regions** 0.61s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 2.20s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.56s 75% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.34s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.27s 50% recall 100% missing '--force'; missing '.env'; missing 'app.log'
...
**Qwen3-0.6B-MLX-4bit / mishear_dev_prod** 0.13s 50% recall 93% missing 'prod'; missing 'middleware'; kept 'proud'; kept 'devin'
