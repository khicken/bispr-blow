...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 98% | 84% | 0.16s | 1.53s | 1.74s | 0/30 | 7/30 | 4/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 0.75s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / self_correction** 0.08s 100% recall 100% ok

> Let's meet at 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.31s 100% recall 96% ok
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.26s 81% recall 94% kept 'like closer'; LOSSY (dropped the last thing said): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.09s 100% recall 94% ok
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.28s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.13s 50% recall 100% kept 'to the left'; kept 'sorry'
...
**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.15s 75% recall 100% kept 'needs to, we'

> The retry logic needs to, we need to first check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.63s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"
...
**Qwen3-0.6B-MLX-4bit / long_doc** 1.48s 92% recall 96% missing 'remembering it'
...
**Qwen3-0.6B-MLX-4bit / long_meeting_notes** 0.06s 92% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.53s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.54s 82% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.70s 62% recall 89% missing 'debate'; missing 'system prompt'; retained 76% < 80%

> So we have two other agents working on Blue Jay Whisper, but I want you to mainly be concerned about the quality of how our transcription works because there's a lot of times where I say a lot of text and it doesn't take into account that I'm trying to prompt, maybe I'm trying to prompt code, and it just changes my text too much and it doesn't fully understand my intent. So council our quality issues, and imagine you're whisper flow, and we're trying to replicate whisper flow one to one.

**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.32s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.24s 94% recall 96% missing 'app.log'

> Check whether flag retry v2 is set in the .env file and if it is, run npm run build --force and tail the logs in var log / app.log.

**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 1.74s 90% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.13s 78% recall 83% missing 'X_audio'
