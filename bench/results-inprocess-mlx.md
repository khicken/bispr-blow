...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 96% | 86% | 0.67s | 1.97s | 1.97s | 0/26 | 7/26 | 4/26 |

## Per case

**Qwen3-0.6B-MLX-4bit / short_question** 0.43s 100% recall 100% ok

> Yeah wait what's in this PR can you see the link again?

**Qwen3-0.6B-MLX-4bit / long_ramble** 1.28s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / terminal_flags** 0.56s 100% recall 88% ok

> Run the tests with --verbose, and then commit and push to dev, not prod.

**Qwen3-0.6B-MLX-4bit / jargon** 0.62s 67% recall 100% missing '.env'

> the work tree is dirty, so i had to stash before rebasing onto main, and the dot env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.43s 67% recall 100% kept 'at 2'

> Let's meet at 2:30 PM tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.68s 100% recall 94% ok

> Okay, so the new build and the login flow are mostly working, but I found two issues: one is the spinner never goes away on slow connections, and another is when you log out, it doesn't clear the session. We should fix those before Friday.

**Qwen3-0.6B-MLX-4bit / question_not_instruction** 0.49s 100% recall 100% ok

> Hey, can you review my PR when you get a chance? It's the one that fixes the webhook retry logic.

**Qwen3-0.6B-MLX-4bit / discourse_like** 0.66s 71% recall 100% kept 'like closer'; LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.41s 90% recall 100% LOSSY: guard rejected it, rules ran instead

> So i think we should just ship it today and see what breaks

**Qwen3-0.6B-MLX-4bit / meaningful_like** 0.51s 100% recall 100% ok

> I like the new design and it looks like a bug in the retry logic. So can you do something like that for the webhook one?

**Qwen3-0.6B-MLX-4bit / false_start** 0.70s 75% recall 91% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.43s 100% recall 71% ok

> Can you move the button to the right side of the toolbar?

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.43s 87% recall 80% kept 'actually'

> We should ship it on Thursday, before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.60s 100% recall 83% ok

> the retry logic needs to check whether the webhook even fired before we retry anything,

**Qwen3-0.6B-MLX-4bit / snap_regions** 1.13s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"
...
**Qwen3-0.6B-MLX-4bit / snap_regions_joined** 1.11s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-0.6B-MLX-4bit / long_doc** 1.54s 86% recall 86% missing 'remembering it'
...
**Qwen3-0.6B-MLX-4bit / long_meeting_notes** 1.40s 100% recall 99% ok
...
**Qwen3-0.6B-MLX-4bit / draft_context** 0.51s 67% recall 100% missing 'FLAG_RETRY_V2'

> So far no errors in sentry and the flag retry v2 flag is on for about 10 percent of orgs.

**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.97s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 1.02s 79% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 1.25s 95% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.81s 86% recall 100% missing 'actually'
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.76s 48% recall 100% missing '--force'; missing '.env'; missing 'app.log'
...
**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 1.49s 93% recall 92% LOSSY: guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / deliberate_repetition** 0.41s 100% recall 100% ok
