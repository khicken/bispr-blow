...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 96% | 83% | 0.31s | 1.62s | 2.18s | 1/30 | 8/30 | 3/30 |

## Per case

**Qwen3-0.6B-MLX-4bit / short_question** 0.12s 100% recall 100% ok

> Yes, wait, what's in this PR? Can you see the link again?

**Qwen3-0.6B-MLX-4bit / long_ramble** 1.11s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / terminal_flags** 0.15s 100% recall 100% ok

> Run the tests with --verbose, and then commit and push to dev, not prod.

**Qwen3-0.6B-MLX-4bit / jargon** 0.18s 67% recall 80% missing '.env'

> The worktree is dirty, so I had to stash before rebasing onto main and the dot env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.31s 100% recall 100% ok

> Let's meet at 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.35s 100% recall 96% ok

> Okay so I tested the new build and the login flow is mostly working, but I found two issues. One is the spinner never goes away on slow connections. Two when you log out it doesn't actually clear the session. So we should fix those before before Friday.
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.56s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.31s 100% recall 92% ok

> We should just ship it today and see what breaks.
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.52s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.36s 50% recall 100% kept 'to the left'; kept 'sorry'

> Can you move the button to the right side of the toolbar, sorry, to the left side?

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.11s 90% recall 80% kept 'actually'

> We should ship it on Thursday actually, before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.39s 100% recall 83% ok
...
**Qwen3-0.6B-MLX-4bit / draft_context** 0.22s 67% recall 100% missing 'FLAG_RETRY_V2'

> So far no errors in Sentry and the flag retry v2 flag is on for about 10 percent of orgs.

**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.96s 91% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.69s 75% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.11s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.39s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.28s 67% recall 100% missing '--force'; missing 'app.log'

> Check whether flag retry v2 is set in the .env file and if it is, run npm run build dash dash force and then tail the logs in var log slash app dot log.

**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 2.18s 95% recall 99% **over 2034ms budget** LOSSY (lost content words): guard rejected it, rules ran instead

> Okay so for the reliability section, I want to start with what actually happens today, when the webhook sender fails, we just log it and move on. So the customer never finds out and neither do we until they email us. Then the second part should be what we're proposing, a dead letter table plus a daily digest that goes to the the org owner listing anything that failed in the last twenty four hours and then third part: talk about the tradeoff, a digest is not real time so if something breaks at nine in the morning they hear about it the next day and I think that's acceptable for now because the alternative is a whole alerting system and we don't we don't have anyone to build that this quarter and then last the the rollout plan is we ship the table first behind a flag measure how often it actually fills up on real traffic and only then decide whether the digest is even worth building.

**Qwen3-0.6B-MLX-4bit / deliberate_repetition** 0.08s 100% recall 100% ok

> Blue Jays, Blue Jays, Blue Jays

**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.38s 100% recall 67% ok
...
**Qwen3-0.6B-MLX-4bit / mishear_control_plain** 0.11s 100% recall 100% ok
