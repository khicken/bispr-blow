...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 84% | 0.32s | 1.53s | 1.85s | 0/30 | 5/30 | 3/30 |

## Per case

**Qwen3-0.6B-MLX-4bit / short_question** 0.11s 100% recall 100% ok

> Yeah, wait what's in this PR can you see the link again?

**Qwen3-0.6B-MLX-4bit / long_ramble** 1.06s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / bug_report** 0.33s 100% recall 94% ok
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.61s 77% recall 98% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.26s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.31s 75% recall 86% kept 'sorry'

> Can you move the button to the right side of the toolbar, sorry?

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.13s 60% recall 100% kept 'friday'; kept 'actually'

> We should ship it on Friday, and actually ship it on Thursday before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.40s 75% recall 100% kept 'needs to, we'
...
**Qwen3-0.6B-MLX-4bit / draft_context** 0.27s 100% recall 88% ok
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.85s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.59s 86% recall 100% kept 'you know'
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.97s 100% recall 96% ok
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.37s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
