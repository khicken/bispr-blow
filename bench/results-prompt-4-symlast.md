...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 98% | 78% | 0.15s | 0.66s | 1.00s | 0/30 | 12/30 | 3/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 0.59s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / terminal_flags** 0.10s 100% recall 100% ok

> Run the tests with --verbose, and then commit and push to dev, not prod.

**Qwen3-0.6B-MLX-4bit / jargon** 0.13s 100% recall 100% ok

> The work tree is dirty so I had to stash before rebasing onto main and the .env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.14s 75% recall 100% kept 'at 2'
...
**Qwen3-0.6B-MLX-4bit / question_not_instruction** 0.13s 100% recall 100% ok

> Hey, can you review my PR when you get a chance, it's the one that fixes the webhook retry logic.

**Qwen3-0.6B-MLX-4bit / discourse_like** 0.22s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.11s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead

> So i think we should just ship it today and see what breaks

**Qwen3-0.6B-MLX-4bit / meaningful_like** 0.15s 100% recall 100% ok

> I like the new design, and it looks like a bug in the retry logic. Can you do something like that for the webhook one.

**Qwen3-0.6B-MLX-4bit / false_start** 0.26s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.15s 75% recall 86% kept 'sorry'
...
**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.16s 80% recall 92% REWORDED: subsequence guard rejected it, rules ran instead; kept 'needs to, we'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead

> The retry logic needs to check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.12s 62% recall 100% kept 'dragon'; kept "shouldn't even be showing"; LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / draft_context** 0.15s 67% recall 94% missing 'FLAG_RETRY_V2'
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.12s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.28s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.21s 50% recall 100% missing '--force'; missing '.env'; missing 'app.log'
...
**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 0.17s 90% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
