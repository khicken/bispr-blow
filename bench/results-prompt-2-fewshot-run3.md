...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 85% | 0.30s | 1.54s | 1.86s | 0/30 | 6/30 | 3/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 1.03s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / jargon** 0.19s 100% recall 100% ok

> The work tree is dirty, so I had to stash before rebasing onto main and the .env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.25s 100% recall 100% ok

> Let's meet at 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.34s 100% recall 96% ok

> Okay so I tested the new build and the login flow is mostly working, but I found two issues. One is the spinner never goes away on slow connections. Two is when you log out it doesn't actually clear the session. We should fix those before Friday.
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.67s 100% recall 83% ok

> Our pill sits a bit too high, when it could be closer to the system tray or the task bar on a desktop, or lower down in the ID or close to the bottom of the screen. It's just too far up and in the middle of everything.

**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.25s 100% recall 92% ok
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.62s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restated_clause** 0.12s 100% recall 80% ok

> We should ship it on Thursday, before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.48s 75% recall 100% kept 'needs to, we'

> The retry logic needs to, we need to first check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.74s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"
...
**Qwen3-0.6B-MLX-4bit / snap_regions_joined** 0.70s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-0.6B-MLX-4bit / long_doc** 1.30s 92% recall 94% missing 'remembering it'
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.86s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.57s 80% recall 100% kept 'you know'

> Note latency is a huge factor we want sub five hundred to six hundred milliseconds in the UI. I'm thinking that there's model options like fastest, medium, and slow, or something like that to show if someone wants to make the choice between three different choices between or maybe it could just be two choices between like you know fast but less accurate or slower and more accurate, whatever you think is the best UI.

**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.85s 86% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.26s 92% recall 97% missing 'app.log'

> Check whether flag retry v2 is set in the .env file and if it is, run npm run build --force and tail the logs in var log / app dot log.

**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 1.52s 95% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.39s 83% recall 83% missing 'X_audio'

> Should our paper even mention X_underscore_audio if we never use it?
...
**Qwen3-0.6B-MLX-4bit / mishear_dev_prod** 0.16s 43% recall 100% missing 'prod'; missing 'middleware'; kept 'proud'; kept 'devin'
