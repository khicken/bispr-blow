...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 80% | 0.18s | 1.08s | 1.38s | 0/30 | 11/30 | 5/30 |

## Per case

**Qwen3-0.6B-MLX-4bit / short_question** 0.08s 100% recall 100% ok

> Yeah wait what's in this PR can you see the link again.

**Qwen3-0.6B-MLX-4bit / long_ramble** 0.57s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / terminal_flags** 0.11s 100% recall 100% ok

> run the tests with --verbose, and then commit and push to dev, not prod.

**Qwen3-0.6B-MLX-4bit / jargon** 0.14s 67% recall 100% missing '.env'

> the work tree is dirty, so i had to stash before rebasing onto main and the dot env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.15s 75% recall 100% kept 'at 2'

> Let's meet at 2 and 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.26s 100% recall 92% ok

> Okay so the new build and login flow are mostly working, but we found two issues: one is the spinner never goes away on slow connections, and another is when you log out, it doesn't actually clear the session. We should fix those before Friday.
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.33s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.12s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.32s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.16s 62% recall 93% kept 'to the left'; kept 'sorry'

> Can you move the button to the right side of the toolbar, sorry, to the left side?

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.09s 60% recall 100% kept 'friday'; kept 'actually'

> We should ship it on Friday, Thursday actually, before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.20s 80% recall 92% REWORDED: subsequence guard rejected it, rules ran instead; kept 'needs to, we'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / snap_regions_joined** 0.59s 71% recall 100% kept 'dragon'; kept "shouldn't even be showing"

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-0.6B-MLX-4bit / long_doc** 1.00s 75% recall 87% missing 'hundred and forty'; missing 'three hundred'; missing 'remembering it'

> Okay so for the design doc, we want to start with the problem. The problem is that our dictation is accurate when you speak clearly, but it falls apart when you mumble or trail off. The cleanup stage can't fix that because by then the words are already wrong. Next section should cover what we actually measured. The transcription tail is about 140 milliseconds, and the cleanup call is around 300. Neither of those is the bottleneck. The bottleneck is recognition quality. Third section: we discussed three options: biasing the recognizer with a vocabulary list, feeding focused field's text in as context, and letting the user correct a word once and remember it. And then the recommendation: we do the third one because it's the only one that gets better. The more you use it, the better it gets, and we should ship it behind a flag so we can measure it on real dictations before turning it on for everyone.

**Qwen3-0.6B-MLX-4bit / long_meeting_notes** 0.13s 92% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / draft_context** 0.15s 67% recall 88% missing 'FLAG_RETRY_V2'

> So, no errors in sentry and the flag retry v2 flag is on for about 10 percent of orgs.

**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.38s 95% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.44s 75% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'you know'; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.19s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.28s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.22s 67% recall 94% missing '--force'; missing '.env'

> check whether flag retry v2 is set in the dot env file and if it is run npm run build dash dash force and then tail the logs in var log slash app.log.

**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 0.18s 90% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.18s 100% recall 67% ok

> Should our paper mention X_audio if we never use it?

**Qwen3-0.6B-MLX-4bit / mishear_frontend_link** 0.07s 20% recall 100% missing 'frontend'; missing 'locally'; kept 'front number'; kept 'cycle'
