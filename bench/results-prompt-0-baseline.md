...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 83% | 0.26s | 1.31s | 2.24s | 1/30 | 9/30 | 2/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 0.94s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.25s 93% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.47s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.35s 100% recall 83% ok
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 0.30s 91% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.09s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 2.24s 89% recall 95% **over 2034ms budget** LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.31s 100% recall 67% ok
...
**Qwen3-0.6B-MLX-4bit / mishear_dev_prod** 0.13s 43% recall 100% missing 'prod'; missing 'middleware'; kept 'proud'; kept 'devin'
