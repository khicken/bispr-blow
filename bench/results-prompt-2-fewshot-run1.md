...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 97% | 81% | 0.27s | 1.51s | 1.56s | 0/30 | 9/30 | 3/30 |
...
**Qwen3-0.6B-MLX-4bit / terminal_flags** 0.13s 100% recall 100% ok

> Run the tests with --verbose, and then commit and push to dev, not prod.

**Qwen3-0.6B-MLX-4bit / jargon** 0.16s 100% recall 100% ok

> The work tree is dirty, so I had to stash before rebasing onto main and the .env file was missing.

**Qwen3-0.6B-MLX-4bit / self_correction** 0.28s 100% recall 100% ok

> Let's meet at 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.33s 100% recall 96% ok
...
**Qwen3-0.6B-MLX-4bit / discourse_like** 0.39s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.28s 100% recall 92% ok

> We should just ship it today and see what breaks.

**Qwen3-0.6B-MLX-4bit / meaningful_like** 0.16s 75% recall 83% missing 'something like that'; retained 58% < 80%

> I like the new design and it looks like a bug in the retry logic.

**Qwen3-0.6B-MLX-4bit / false_start** 0.37s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.31s 62% recall 93% kept 'sorry'

> Can you move the button to the right side of the toolbar, sorry?

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.10s 100% recall 80% ok

> We should ship it on Thursday, before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.28s 100% recall 83% ok

> The retry logic needs to check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.46s 62% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'dragon'; kept "shouldn't even be showing"; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / snap_regions_joined** 0.47s 62% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; kept 'dragon'; kept "shouldn't even be showing"; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-0.6B-MLX-4bit / long_doc** 1.35s 92% recall 92% missing 'remembering it'
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 1.44s 91% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_ui_options** 0.46s 75% recall 100% kept 'you know'; LOSSY (dropped the last thing said): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.58s 42% recall 82% missing 'debate'; missing 'system prompt'; missing 'one to one'; retained 72% < 80%

> So we have two other agents working on Blue Jay Whisper. I want you to mainly be concerned about the quality of how our transcription works because there's a lot of times where I say a lot of text and it doesn't take into account that I'm trying to prompt maybe I'm trying to prompt code and it just changes my text too much and it doesn't fully understand my intent. Council our quality issues. Imagine you're Whisper Flow and we're trying to replicate Whisper Flow one-to-one.

**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.25s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.23s 58% recall 100% missing '--force'; missing 'app.log'; missing 'npm run build'

> Check whether flag retry v2 is set in the .env file and if it is, run npm build dash dash force and tail the logs in var log / app / dot log.

**Qwen3-0.6B-MLX-4bit / design_doc_polish_long** 1.56s 95% recall 99% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.25s 67% recall 100% missing 'X_audio'

> Should our paper even mention X_underscore audio if we never use it?
...
**Qwen3-0.6B-MLX-4bit / mishear_control_plain** 0.09s 100% recall 100% ok
