...
|---|---|---|---|---|---|---|---|---|
| Qwen3-0.6B-MLX-4bit | 98% | 77% | 0.15s | 0.76s | 1.06s | 0/30 | 12/30 | 2/30 |
...
**Qwen3-0.6B-MLX-4bit / long_ramble** 0.68s 88% recall 100% REWORDED: subsequence guard rejected it, rules ran instead; LOSSY (reworded content on the coding path): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / self_correction** 0.17s 75% recall 100% kept 'at 2'

> Let's meet at 2 and 3 pm tomorrow at the office.

**Qwen3-0.6B-MLX-4bit / bug_report** 0.29s 94% recall 96% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / question_not_instruction** 0.15s 100% recall 100% ok

> Hey, can you review my PR when you get a chance? It's the one that fixes the webhook retry logic.

**Qwen3-0.6B-MLX-4bit / discourse_like** 0.31s 71% recall 100% kept 'like closer'; LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / stacked_fillers** 0.13s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / false_start** 0.30s 75% recall 100% kept 'like center'
...
**Qwen3-0.6B-MLX-4bit / restart_wrong_word** 0.20s 62% recall 93% kept 'sorry'

> Can you move the button to the right side of the toolbar, sorry, to the right side of the toolbar.

**Qwen3-0.6B-MLX-4bit / restated_clause** 0.10s 60% recall 100% kept 'friday'; kept 'actually'

> We should ship it on Friday, Thursday actually, before the demo.

**Qwen3-0.6B-MLX-4bit / abandoned_phrase** 0.18s 100% recall 83% ok

> The retry logic needs to check whether the webhook even fired before we retry anything.

**Qwen3-0.6B-MLX-4bit / snap_regions** 0.13s 62% recall 100% kept 'dragon'; kept "shouldn't even be showing"; LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / snap_regions_joined** 0.12s 62% recall 100% kept 'dragon'; kept "shouldn't even be showing"; LOSSY (dropped content): guard rejected it, rules ran instead

> When we do the dragon, it's supposed to show the 3 regions that will automatically snap the dragon too. We shouldn't it shouldn't move. We shouldn't even have the pill picked up until it's snapped into the other 2 possible positions if that makes sense. Like currently we can pick it up and then it'll snap, but we shouldn't even be showing, it shouldn't even be shown the pill is being picked up.

**Qwen3-0.6B-MLX-4bit / long_doc** 1.06s 83% recall 88% missing 'remembering it'

> Okay so for the design doc, we want to start with the problem. The problem is that our dictation is accurate when you speak clearly but falls apart when you mumble or trail off. The cleanup stage can't fix that because by then the words are already wrong. Next, we want to cover what we actually measured. The transcription tail is around a hundred and forty milliseconds, and the cleanup call is around three hundred. Neither of those is the bottleneck. The bottleneck is recognition quality. Next, we want to lay out the three options we discussed: biasing the recognizer with a vocabulary list, feeding the focused field's text in as context, and letting the user correct a word once and remember it. Finally, the recommendation is we do the third one because it's the only one that gets better as we use it and we should ship it behind a flag so we can measure it on real dictations before turning it on for everyone.

**Qwen3-0.6B-MLX-4bit / long_meeting_notes** 0.12s 92% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_streaming** 0.20s 91% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / agent_prompt_council** 0.13s 86% recall 100% LOSSY (dropped content): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / enumerated_options** 0.29s 88% recall 100% LOSSY (lost content words): guard rejected it, rules ran instead
...
**Qwen3-0.6B-MLX-4bit / identifier_symbols** 0.20s 50% recall 100% missing '--force'; missing '.env'; missing 'app.log'
...
**Qwen3-0.6B-MLX-4bit / spoken_underscore** 0.15s 33% recall 83% missing 'X_audio'; kept 'X underscore'
...
**Qwen3-0.6B-MLX-4bit / mishear_control_plain** 0.07s 100% recall 100% ok

> Can you check the front door is locked before you leave.

**Qwen3-0.6B-MLX-4bit / mishear_dev_prod** 0.11s 43% recall 100% missing 'prod'; missing 'middleware'; kept 'proud'; kept 'devin'
